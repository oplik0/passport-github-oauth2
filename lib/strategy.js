  ...
  (async (accessToken) => {
    ...
    try {
      ...
      if (json && json.error && json.error.message) {
        ...
        return {
          error: new UserInfoError(json.error_description, json.error),
        };
      }
      return {
        error: new InternalOAuthError("Failed to fetch user profile", err),
      };
    }

    json = body.data;
    if (
      this._scope.some((scope) => scope === "user" || scope === "user:email")
    ) {
    ...
    if (
      this._scope.some((scope) => scope === "read:org" || scope === "admin:org")
    ) {
